...
var BIND_VAR = 1 // Var-style binding
var BIND_LEXICAL = 2 // Let- or const-style binding
var BIND_FUNCTION = 3 // Function declaration
var BIND_SIMPLE_CATCH = 4 // Simple (identifier pattern) catch binding
...
Parser.extend = function extend () {
  var plugins = []; var len = arguments.length
...
pp.strictDirective = function (start) {
  for (;;) {
    ...
    if ((match[1] || match[2]) === 'use strict') {
      ...
      var spaceAfter = skipWhiteSpace.exec(this.input); var end = spaceAfter.index + spaceAfter[0].length
...
var loopLabel = { kind: 'loop' }; var switchLabel = { kind: 'switch' }

pp$1.isLet = function (context) {
  if (this.options.ecmaVersion < 6 || !this.isContextual('let')) { return false }
  skipWhiteSpace.lastIndex = this.pos
  var skip = skipWhiteSpace.exec(this.input)
  var next = this.pos + skip[0].length; var nextCh = this.input.charCodeAt(next)
...
pp$1.parseStatement = function (context, topLevel, exports) {
  var starttype = this.type; var node = this.startNode(); var kind
  ...
  switch (starttype) {
    ...
    default:
      ...
      var maybeName = this.value; var expr = this.parseExpression()
...
pp$1.parseForStatement = function (node) {
  ...
  if (this.type === types._var || this.type === types._const || isLet) {
    var init$1 = this.startNode(); var kind = isLet ? 'let' : this.value
    this.next()
...
var FUNC_STATEMENT = 1; var FUNC_HANGING_STATEMENT = 2; var FUNC_NULLABLE_ID = 4
...
pp$1.parseFunction = function (node, statement, allowExpressionBody, isAsync) {
  ...
  var oldYieldPos = this.yieldPos; var oldAwaitPos = this.awaitPos; var oldAwaitIdentPos = this.awaitIdentPos
  this.yieldPos = 0
  this.awaitPos = 0
...
pp$1.parseClassElement = function (constructorAllowsSuper) {
  ...
  var tryContextual = function (k, noLineBreak) {
    ...
    var start = this$1.start; var startLoc = this$1.startLoc
...
pp$3.parseExpression = function (noIn, refDestructuringErrors) {
  var startPos = this.start; var startLoc = this.startLoc
...
pp$3.parseMaybeAssign = function (noIn, refDestructuringErrors, afterLeftParse) {
  ...
  var ownDestructuringErrors = false; var oldParenAssign = -1; var oldTrailingComma = -1
  if (refDestructuringErrors) {
  ...
  if (this.type.isAssign) {
    ...
    if (refDestructuringErrors.shorthandAssign >= node.left.start) { refDestructuringErrors.shorthandAssign = -1 } // reset because shorthand default was used correctly
    this.checkLVal(left)
    this.next()
...
pp$3.parseMaybeUnary = function (refDestructuringErrors, sawUnary) {
  var startPos = this.start; var startLoc = this.startLoc; var expr
  if (this.isContextual('await') && (this.inAsync || (!this.inFunction && this.options.allowAwaitOutsideFunction))) {
    expr = this.parseAwait()
    sawUnary = true
  } else if (this.type.prefix) {
    var node = this.startNode(); var update = this.type === types.incDec
    node.operator = this.value
    node.prefix = true
    this.next()
...
pp$3.parseSubscript = function (base, startPos, startLoc, noCalls, maybeAsyncArrow, optionalChained) {
  ...
  if (computed || (optional && this.type !== types.parenL && this.type !== types.backQuote) || this.eat(types.dot)) {
    ...
    var refDestructuringErrors = new DestructuringErrors(); var oldYieldPos = this.yieldPos; var oldAwaitPos = this.awaitPos; var oldAwaitIdentPos = this.awaitIdentPos
    this.yieldPos = 0
    this.awaitPos = 0
    this.awaitIdentPos = 0
...
pp$3.parseExprAtom = function (refDestructuringErrors) {
  ...
  var node; var canBeArrow = this.potentialArrowAt === this.start
  switch (this.type) {
    case types._super:
    ...
    case types.name:
      var startPos = this.start; var startLoc = this.startLoc; var containsEsc = this.containsEsc
    ...
    case types.parenL:
      var start = this.start; var expr = this.parseParenAndDistinguishExpression(canBeArrow)
      if (refDestructuringErrors) {
...
pp$3.parseParenAndDistinguishExpression = function (canBeArrow) {
  var startPos = this.start; var startLoc = this.startLoc; var val; var allowTrailingComma = this.options.ecmaVersion >= 8
  if (this.options.ecmaVersion >= 6) {
    this.next()

    var innerStartPos = this.start; var innerStartLoc = this.startLoc
    var exprList = []; var first = true; var lastIsComma = false
    var refDestructuringErrors = new DestructuringErrors(); var oldYieldPos = this.yieldPos; var oldAwaitPos = this.awaitPos; var spreadStart
    this.yieldPos = 0
    this.awaitPos = 0
    ...
    var innerEndPos = this.start; var innerEndLoc = this.startLoc
    this.expect(types.parenR)
...
pp$3.parseNew = function () {
  ...
  var startPos = this.start; var startLoc = this.startLoc; var isImport = this.type === types._import
...
pp$3.parseObj = function (isPattern, refDestructuringErrors) {
  var node = this.startNode(); var first = true; var propHash = {}
  node.properties = []
  this.next()
  while (!this.eat(types.braceR)) {
    if (!first) {
...
pp$3.parseProperty = function (isPattern, refDestructuringErrors) {
  var prop = this.startNode(); var isGenerator; var isAsync; var startPos; var startLoc
...
pp$3.parseMethod = function (isGenerator, isAsync, allowDirectSuper) {
  var node = this.startNode(); var oldYieldPos = this.yieldPos; var oldAwaitPos = this.awaitPos; var oldAwaitIdentPos = this.awaitIdentPos
...
pp$3.parseFunctionBody = function (node, isArrowFunction, isMethod) {
  ...
  var oldStrict = this.strict; var useStrict = false
...
pp$9.skipBlockComment = function () {
  ...
  var start = this.pos; var end = this.input.indexOf('*/', this.pos += 2)
...
pp$9.readRegexp = function () {
  var escaped; var inClass; var start = this.pos
  for (;;) {
...
pp$9.readInt = function (radix, len) {
  var start = this.pos; var total = 0
  for (var i = 0, e = len == null ? Infinity : len; i < e; ++i) {
    var code = this.input.charCodeAt(this.pos); var val = (void 0)
...
pp$9.readCodePoint = function () {
  var ch = this.input.charCodeAt(this.pos); var code
...
pp$9.readString = function (quote) {
  var out = ''; var chunkStart = ++this.pos
  for (;;) {
...
pp$9.readWord1 = function () {
  this.containsEsc = false
  var word = ''; var first = true; var chunkStart = this.pos
